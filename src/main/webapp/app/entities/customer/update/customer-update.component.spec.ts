...
describe('Customer Management Update Component', () => {
  ...
  describe('ngOnInit', () => {
    it('Should call Product query and add missing value', () => {
      ...
      const favoriteProducts: IProduct[] = [{ id: 17307 }];
      customer.favoriteProducts = favoriteProducts;

      const productCollection: IProduct[] = [{ id: 10770 }];
      jest.spyOn(productService, 'query').mockReturnValue(of(new HttpResponse({ body: productCollection })));
      const additionalProducts = [...favoriteProducts];
    ...
    it('Should update editForm', () => {
      const customer: ICustomer = { id: 456 };
      const favoriteProducts: IProduct = { id: 12756 };
      customer.favoriteProducts = [favoriteProducts];

      activatedRoute.data = of({ customer });
      comp.ngOnInit();

      expect(comp.productsSharedCollection).toContain(favoriteProducts);
