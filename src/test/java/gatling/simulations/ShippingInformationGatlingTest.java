...
/**
 * Performance test for the ShippingInformation entity.
 */
public class ShippingInformationGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all shippingInformations")
                ...
                .exec(
                    http("Create new shippingInformation")
                        .post("/api/shipping-informations")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"deliveryAddress\": \"SAMPLE_TEXT\"" + ", \"shippingStatus\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_shippingInformation_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(http("Get created shippingInformation").get("${new_shippingInformation_url}").headers(headers_http_authenticated))
                        .pause(10)
                )
                .exec(
                    http("Delete created shippingInformation").delete("${new_shippingInformation_url}").headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the ShippingInformation entity").exec(scn);
